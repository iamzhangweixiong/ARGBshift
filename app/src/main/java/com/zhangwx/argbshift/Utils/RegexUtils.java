...
//    private static final String REGEX_DIGIT = "\\-?[1-9]\\d+";
    ...
    private static final String REGEX_IP_ADDRESS = "[1-9](\\d{1,2})?\\.(0|([1-9](\\d{1,2})?))\\.(0|([1-9](\\d{1,2})?))\\.(0|([1-9](\\d{1,2})?))";
    private static final String REGEX_BLANK_SPACE = "\\s+";
    private static final String REGEX_COMPLETE_DOMAIN = "[^//]*?\\.(com|cn|net|org|biz|info|cc|tv)";
    ...
    public static boolean checkDecimals(String decimals) {
        return Pattern.matches(REGEX_DECIMALS, decimals);
    }
    ...
    public static boolean checkBlankSpace(String blankSpace) {
        return Pattern.matches(REGEX_BLANK_SPACE, blankSpace);
    }
     ...
     * <pre>
     * 获取网址 URL 的完整域名
     * http://detail.tmall.com/item.htm?spm=a230r.1.10.44.1xpDSH&id=15453106243&_u=f4ve1uq1092 ->> detail.tmall.com
    ...
    public static String getCompleteDomain(String url) {
        Pattern p = Pattern.compile(REGEX_COMPLETE_DOMAIN, Pattern.CASE_INSENSITIVE);
        Matcher matcher = p.matcher(url);
        matcher.find();
        return matcher.group();
    }
